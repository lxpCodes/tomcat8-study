...
public final class StandardServer extends LifecycleMBeanBase implements Server {
    ...
    protected void initInternal() throws LifecycleException {
        ...
        //尽管这个cache是全局的,但是如果在同一个jvm中存在多个server,那么会注册多个不同名字的String Cache,这种情况在内嵌Tomcat中可能会出现
        log.info("向jmx中注册全局的String cache");
        onameStringCache = register(new StringCache(), "type=StringCache");

        // Register the MBeanFactory
        log.info("注册MBeanFactory,用来管理Server");
        MBeanFactory factory = new MBeanFactory();
        factory.setContainer(this);
        onameMBeanFactory = register(factory, "type=MBeanFactory");

        // Register the naming resources
        log.info("向jmx中注册全局的NamingResource");
        ...
        if (getCatalina() != null) {
            ...
            // Walk the class loader hierarchy. Stop at the system class loader.遍历类加载器层次结构。在系统类加载器处停止。
            // This will add the shared (if present) and common class loaders 这将添加共享(如果存在)和公共类加载器
        ...
        log.info("在这里调用所有StandardService进行初始化");
        for (int i = 0; i < services.length; i++) {
            services[i].init();
        }
    }
